...
public class JSONHelper
{
	...
	private final String	TAG_BERITA	= "berita";
	private final String	TAG_ID			= "id_berita";
	private final String	TAG_JUDUL		= "judul";
	...
	public ArrayList<Berita> getBeritaAll(JSONObject jobj)
	{
		ArrayList<Berita> listTempatMakan = new ArrayList<Berita>();

		try
		{
			JSONArray arrayBerita = jobj.getJSONArray(TAG_BERITA);

			for (int i = 0; i < arrayBerita.length(); i++)
			{
				JSONObject jobject = arrayBerita.getJSONObject(i);

				Log.d("log", "muter ke " + i);
				listTempatMakan.add(new Berita(jobject.getInt(TAG_ID), jobject.getString(TAG_JUDUL), jobject
